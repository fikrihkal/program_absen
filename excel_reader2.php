...
function gmgetdate($tss = null){
	$kkk = array('seconds','minutes','hours','mday','wday','mon','year','yday','weekday','month',0);
...
class OLERead {
	...
	function __readPropertySets(){
		...
		while ($offset < strlen($this->entry)) {
			$ddd = substr($this->entry, $offset, PROPERTY_STORAGE_BLOCK_SIZE);
...
class Spreadsheet_Excel_Reader {
	...
	function color($row,$col,$sheet=0) {
		...
		$cii = $this->fontProperty($row,$col,$sheet,'color');
                return $this->rawColor($cii);
        }
        function rawColor($cii) {
		if (($cii <> 0x7FFF) && ($cii <> '')) {
			return $this->colors[$cii];
		}
		return "";
	}
	...
	function _format_value($format,$num,$fff) {
		// 49==TEXT format
		// http://code.google.com/p/php-excel-reader/issues/detail?id=7
		if ( (!$fff && $format=="%s") || ($fff==49) || ($format=="GENERAL") ) { 
		...
		if (preg_match($number_regex,$pattern,$matches)) {
			$unused_left = $matches[1];
			$unused_dec = $matches[2];
			$right = $matches[3];
			if ($has_commas) {
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
						...
						$xff = array();
						$xff['formatIndex'] = $indexCode;
						$xff['align'] = $align;
						$xff['fontIndex'] = $fontIndexCode;
						$xff['bgColor'] = $bgcolor;
						$xff['fillPattern'] = $fillPattern;

						$border = ord($data[$pos+14]) | (ord($data[$pos+15]) << 8) | (ord($data[$pos+16]) << 16) | (ord($data[$pos+17]) << 24);
						$xff['borderLeft'] = $this->lineStyles[($border & 0xF)];
						$xff['borderRight'] = $this->lineStyles[($border & 0xF0) >> 4];
						$xff['borderTop'] = $this->lineStyles[($border & 0xF00) >> 8];
						$xff['borderBottom'] = $this->lineStyles[($border & 0xF000) >> 12];
						
						$xff['borderLeftColor'] = ($border & 0x7F0000) >> 16;
						$xff['borderRightColor'] = ($border & 0x3F800000) >> 23;
						$border = (ord($data[$pos+18]) | ord($data[$pos+19]) << 8);

						$xff['borderTopColor'] = ($border & 0x7F);
						$xff['borderBottomColor'] = ($border & 0x3F80) >> 7;
												
						if (array_key_exists($indexCode, $this->dateFormats)) {
							$xff['type'] = 'date';
							$xff['format'] = $this->dateFormats[$indexCode];
							if ($align=='') { $xf['align'] = 'right'; }
						}elseif (array_key_exists($indexCode, $this->numberFormats)) {
							$xff['type'] = 'number';
							$xff['format'] = $this->numberFormats[$indexCode];
							if ($align=='') { $xf['align'] = 'right'; }
						}else{
							$isdate = FALSE;
							$formatstr = '';
							...
							if ($isdate){
								$xff['type'] = 'date';
								$xff['format'] = $formatstr;
								if ($align=='') { $xff['align'] = 'right'; }
							}else{
				...
				case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						...
						$unused_rec_typeFlag = ord($data[$pos+8]);
						$unused_rec_visibilityFlag = ord($data[$pos+9]);
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
					...
					for ($i = 0; $i < $cellRanges; $i++) {
						$frr =  ord($data[$spos + 8*$i + 2]) | ord($data[$spos + 8*$i + 3])<<8;
						$lrr =  ord($data[$spos + 8*$i + 4]) | ord($data[$spos + 8*$i + 5])<<8;
						$fcc =  ord($data[$spos + 8*$i + 6]) | ord($data[$spos + 8*$i + 7])<<8;
						$lcc =  ord($data[$spos + 8*$i + 8]) | ord($data[$spos + 8*$i + 9])<<8;
						if ($lrr - $frr > 0) {
							$this->sheets[$this->sn]['cellsInfo'][$frr+1][$fcc+1]['rowspan'] = $lrr - $frr + 1;
						}
						if ($lcc - $fcc > 0) {
							$this->sheets[$this->sn]['cellsInfo'][$frr+1][$fcc+1]['colspan'] = $lcc - $fcc + 1;
						}
					}
					break;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_MULRK:
					...
					for ($i = 0; $i < $columns; $i++) {
						...
						$info = $this->_getCellDetails($tmppos-4,$numValue,$colFirst + $iii + 1);
						$tmppos += 6;
						$this->addcell($row, $colFirst + $iii, $info['string'], $info);
					}
					break;
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_STRING:
					...
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
						...
						if ($richString) {
							// Read in the crun
							$unused_formattingRuns =ord($data[$xpos]) | (ord($data[$xpos+1]) << 8);
							$xpos += 2;
						}
						if ($extendedString) {
							// Read in cchExtRst
							$unused_extendedRunLength =$this->_GetInt4d($this->data, $xpos);
							$xpos += 4;
						}
						$len = ($asciiEncoding)?$numChars : $numChars*2;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
					...
					$cww = ord($data[$spos+4]) | ord($data[$spos+5]) << 8; 
					$cxf = ord($data[$spos+6]) | ord($data[$spos+7]) << 8; 
					$coo = ord($data[$spos+8]); 
					for ($coli = $colfrom; $coli <= $colto; $coli++) {
						$this->colInfo[$this->sn][$coli+1] = Array('width' => $cww, 'xf' => $cxf, 'hidden' => ($coo & 0x01), 'collapsed' => ($coo & 0x1000) >> 12);
					}
					break;
